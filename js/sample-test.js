...
function getSampleExams() {
  ...
  pElem.innerHTML =
    '<center class="ldngsmpltst">Loading... <div id="loaderfls" class="spinner-border text-light" style="display: inline-block;"></div></center>';
  $("body").append(pElem);
...
function getExams(ownap) {
  ...
  var ur2 =
    "AKfycbxa777LU5u9jO-pRtylDoFw0kKMz3ES5O7inA4XqJeBHfgtixiulk2hQFCkd4O2EmTc";
  var url =
    ur1 +
    ur2 +
    "/exec" +
    "?callback=ctrlqexms&emp=" +
    ownap +
    "&action=getexms";
...
function ctrlqexms(e) {
  ...
  for (var k = samlen; k >= 1; k--) {
    ...
    var htm = (n2Elem.innerHTML +=
      ...
      tsdt.ExamTitle +
      "</div>" +
      "<div class='tdttw'><span>Exam Description : </span>" +
      tsdt.ExamDescp +
      "</div>" +
      "<div class='tdtth'><span>Duration : </span>" +
      tsdt.TDuration +
      "</div>" +
      "<div class='tdtfr'><span>Educator Name : </span>" +
      tsdt.EducatorName +
      "</div></div>" +
      "<div class='col-sm-4'><button class='btn btn-primary tksmpexm' onclick='startsmpexm(this)'>Start Exam</button>" +
      "<input value='" +
      tsdt.ExamID +
      "' class='smexmid' style='display:none;'>" +
      "<input value='" +
      tsdt.PKey +
      "' class='smexmpk' style='display:none;'>" +
      "<input value='" +
      tsdt.ExamPass +
      "' class='smexmps' style='display:none;'>" +
      "</div>" +
      "</div></div></div>");
  }
  n1Elem.innerHTML =
    "<center><div style='max-width:800px;'>" + htm + "</div></center>";
  var dtelm = document.createElement("input");
  dtelm.id = "smdt";
  dtelm.value = JSON.stringify(samdata);
  $("body").append(dtelm);
  $(".smprtlTwo").show();
}
...
$(document).on("keyup search", "#srchsamtest", function () {
  ...
  if (!found) {
    if (!$("#noTodFound").length) {
      $(".smprtlOne").append(
        '<div id="noTodFound" class="condata" style="text-align:center;padding:35px;color:#000;font-weight:500;background:#bcb6f8;width:95%;max-width:800px;height:100px;">No Sample Exam found</div>',
      );
    }
  } else {
    $("#noTodFound").remove();
...
function checkresultsmp() {
  ...
  document.getElementById("crtanssmp").innerHTML =
